...
####    CODE HERE: Compute the CHANGE in displacement of each wheel
    # REPLACE the zeros with the proper expressions. 
    diff_left = d_left - d_left_previous # v_l hat in lecture notes
    diff_right = d_right - d_right_previous # v_r hat in lecture notes
...
####    CODE HERE: compute change in path length and change in angle
    ...
    center_velocity = (diff_left + diff_right) / 2
    omega = (diff_right - diff_left) / wheel_width
    
    diff_pathlength = center_velocity
    diff_theta = omega
...
####    CODE HERE: compute the change in position and heading according to the dead-reckoning equations
    ...
    # y_hat_bf_avg = (
    #     -np.sin(theta_avg)*r_center_world_estimated[0],
    #     np.cos(theta_avg)*r_center_world_estimated[1]
    # )

    # r_center_world_estimated[0] = r_center_world_estimated[0] + diff_pathlength*y_hat_bf_avg[0]      # x-direction position
    # r_center_world_estimated[1] = r_center_world_estimated[1] + diff_pathlength*y_hat_bf_avg[1]      # y-direction position

    r_center_world_estimated[0] = r_center_world_estimated[0] + -diff_pathlength* np.sin(theta_avg)
    r_center_world_estimated[1] = r_center_world_estimated[1] + diff_pathlength*np.cos(theta_avg)
